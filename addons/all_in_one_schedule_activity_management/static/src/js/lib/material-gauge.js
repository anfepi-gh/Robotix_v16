...
// Ejecutar el código solo después de que el DOM esté listo
document.addEventListener('DOMContentLoaded', function() {
    var gaugeElement = document.querySelector('.gauge');
    if (gaugeElement) { // Verifica si gaugeElement existe en el DOM
        var gauge = Gauge(gaugeElement);
        gauge.value(0.75); // Establece el valor del gauge en 75%
    } else {
        console.error("Elemento '.gauge' no encontrado en el DOM");
    }
});
